...
!
! to compile:
! gfortran -std=f95 -fall-intrinsics -O3 Sequoia_SA.f90 -o Seq
! to debug:
! gfortran -std=f95 -fall-intrinsics -Wall -pedantic -fbounds-check -g -Og Sequoia_SA.f90 -o Seq
!
! ####################################################################
! @@@@   MODULES   @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
! ####################################################################
...
if (quiet < 1) then
  ...
  write(*, '(" @ Birth year    :  min* - max: ", 2i5, ", Max age parent: ", i5)') &
    BYzero +1, BYzero +nYears, maxAgePO
...
integer :: i, j, k, x, topX, Anci(2,mxA), Ancj(2,mxA), ADX, maybe, Lboth, &
    ...
    if (ParSib < 2) then
      ...
      do k=1,2 
        ...
        if (AgeDiff(i,j)>=0) then
          call CheckPair(i, j, k, 6, LL, LLX)  
        else
          call CheckPair(j, i, k, 6, LL, LLX)
        endif
        call BestRel2(LL, topX, dLL) 
...
if (dble(OppHomM(i,j))/dble(Lboth)  > 2.0*dble(MaxOppHom)/dble(nSnp))  return
...
! if (A==-42 .and. P1==-4 .and. P2==-7)  then
  ! open (unit=42,file='log.txt',status="unknown", position="append")
  ! write(42,*) ""
  ! write(42,*) "CalcPX2:"
  ! do x=1,2
    ! write(42,'(2f8.1, "   ", 2f8.1)')  LLY(x,:),  LLcor(x,:)
  ! enddo
  ! close(42)
! endif
...
!      if (OppHomM(i,j) > maxOppHom .or. OppHomM(i,j)<0) cycle   ! implied by LLR_O
    if (LLR_O(i,j) < TF .or. LLR_O(i,j)==missing) cycle  
    if (AgeDiff(i,j) <= 0)  cycle  
    call ChkValidPar(i,sex(i), j,sex(j), ParOK)
    if (.not. ParOK)  cycle
...
double precision :: LRS, LLRX(mxCP,mxCP), LLRY(mxCP,mxCP), ALR,   & 
  LLRZpair(mxCP,mxCP,2), LLRZsingle(2,mxCP,2), gLL(4,2), TAx, dLLrev(2), LLA(7,7,2,2), LRStmp(2,2)
logical ::  AgeAUnk, SexUnk(mxCP, 2), MonoPair(mxCP, mxCP), emptySibship(mxCP, 2), &
  PairCand(mxCP, mxCP), AgeUnk(2), maybeRev(2), DoSingle, SingleCand(mxCP,2), DoLog
...
if (A > 0) then
  fcl = 1
! if (A==995 .or. A==686)   DoLog = .TRUE.
else !if (A < 0) then
  fcl = 4
!  if (any(SibID(:,-A, kAIN) == 94) .and. kAIN==2)  DoLog = .TRUE.
endif
logfile = "log.txt"
...
if (DoLog) then
  ...
  do m=1,2
    do u=1, nCP(m)
      x=CandPar(u,m)
      if (x<0)  write(42, '(2i6, ": ", 2i6, ", ", 50i6)')  m, x, GpID(:,-x,m), SibID(1:ns(-x,m), -x,m)
    enddo 
    write(42,*) ""
  enddo
  close(42)
...
if (ANY(PairCand)) then  ! possibly parent-pair  
  do u=1, nCP(1)
    do v=1, nCP(2)
      if (.not. PairCand(u,v))  cycle   !  .and. COUNT(PairCand)>1
    ...
    do v=1, nCP(2)
      ...
      if (AgeAUnk .or. any(AgeUnk)) then                                  
        do m=1,2
          ...
          call SetEstBY(A, kA)  
!      if (DoLog)  print *, "rev? ", m, all(dLLrev==missing), AgePhase, LLRZpair(u,v,:) - dLLrev
          if (maybeRev(m) .and. ( &
...
if (COUNT(SingleCand) >= 1) then
  do m=1,2
    do u=1,nCP(m)
      ...
      if (COUNT(SingleCand)==1 .or. Complx==0) then   ! do not also condition on other CP being parent
        ...
        do n=1,2 
          do v=1,nCP(n)
            if (.not. SingleCand(v,n))  cycle
            j = (n-1)*mxCP + v
            if (j >= i)  exit  ! only do lower triangle of all singles X all singles matrix
            if (m==1 .and. n==2 .and. PairCand(u,v))  cycle  ! already done
            if (CandPar(u,m) == CandPar(v,n))  cycle  ! when sex=4 
...
do AG=1,2
 ! B + curPar(3-kB)
  ...
  LLP(3   ,2,AG) = MaxLL((/RESHAPE(LLA(:,:,3-kB,AG), (/7*7/)), &
      RESHAPE(LLA(notfcl,:,kB,AG),(/6*7/))/))

  ! only B (| curPar(kB))
  LLP(kB  ,1,AG) = MaxLL(RESHAPE(LLA(fcl,:,:,AG), (/2*7/)))
  LLP(kB  ,2,AG) = MaxLL(RESHAPE(LLA(notfcl,:,:,AG),(/2*6*7/))) 
  if ((LLP(kB,1,AG) - LLP(kB,2,AG)) > (LLA(fcl,7,3-kB,AG) - MaxLL(LLA(notfcl,7,3-kB,AG))) .and. &
    LLA(fcl,7,3-kB,AG) < 0) then
  ...
  if (curPar(3-kB)/=0) then
    LLP(3-kB,1,AG) = MaxLL((/LLA(:,fcl,3-kB,AG), RESHAPE(LLA(:,:,kB,AG), (/7*7/))/))
    LLP(3-kB,2,AG) = MaxLL(RESHAPE(LLA(:,notfcl,3-kB,AG),(/6*7/))) 
    if ((LLP(3-kB,1,AG) - LLP(3-kB,2,AG)) > (LLA(7,fcl,3-kB,AG) - MaxLL(LLA(7,notfcl,3-kB,AG)))) then
      LLP(3-kB,1,AG) = LLA(7,fcl,3-kB,AG)
      LLP(3-kB,2,AG) = MaxLL(LLA(7,notfcl,3-kB,AG)) 
    endif
  endif
enddo
...
integer :: focal, m, x, curPar(2), mid(5), parB(2), curGG(2), r, fclx(3), z
...
if (ALL(curPar==0)) then
  ...
  do m=1,2 ! sex currently assigned par
    ...
    if (A>0 .and. curPar(m) == parB(m) .and. curPar(m)/=0) then
      LLA(2:3,7,m,:) = AlreadyAss  ! HS implies curPar = Par
      if (curPar(3-m) == parB(3-m) .and.  CurPar(3-m)/= 0) then
        LLA(2,7,m,:) = AlreadyAss
      endif
    endif
    if (Complx==0) then
    ...
    if (m/=kB .and. A<0) then  
      ...
      if (ALL(ALRtmp/=impossible)) then
        ...
        if (LLtmp + LLU(3-m) > LLA(6,6,m,1) .or. LLA(6,6,m,1)>0) then
          LLA(6,6,m,1) = LLtmp + LLU(3-m)  
          LLA(6,6,m,2) = LLA(6,6,m,1) + ALRtmp(1) + ALRtmp(2)  
        endif
      endif
    endif   
    ...
    if (A>0 .and. B>0 .and. curPar(m)>0 .and. B/=curPar(m)) then
      ...
      call trioGP(A, B,kB, curPar(m),m, LLtrio(4))  ! B & curPar(m) both GP of A
        ...
        if (all(ALRtmp/=impossible) .or. (r>3 .and. any(ALRtmp/=impossible))) then
          ...
          do x=1,2
            if (ALRtmp(x)/=impossible) then
              LLA(r,r,m,2) = LLA(r,r,m,2) + ALRtmp(x)
            endif
          enddo
        endif
      enddo
      
!      call CalcAgeLR(B,kB, A,kA, kA,1, .FALSE., ALRtmp(1))
!      call CalcAgeLR(CurPar(m),m, A,kA, kA,1, .FALSE., ALRtmp(2))
!      if (all(ALRtmp/=impossible)) then
!        call trioOff(A,kA, B, curPar(m), LLtrio(7))  ! B & curPar(m) both offspring of A      
!        LLA(m,6,5,:) = LLtrio(7) + LLU(3-m)
!        LLA(6,5,m,2) = LLA(6,5,m,2) + ALRtmp(1) + ALRtmp(2)
!      endif
    ...
    if (m/=kB .and. curGG(kA)==0 .and. (A>0 .or. B>0)) then  ! A>0 .and. B<0 .and. curPar(m)>0 .and. 
      ...
      if (ParOK) then 
        ...
        if (ParOK) then
          ...
          if (A>0) then
            call CalcU(B,kB, curPar(m),m, LLtmp)
            LLA(6,2,m,1) = LLtmp + LLU(3-m)  
          else if (B>0) then
            call CalcU(A,kA, curPar(m),m, LLtmp)
            LLA(6,2,m,1) = LLtmp + LLU(3)
          endif          
  ...
  ! open (unit=69,file="log-CalcCPL.txt",status="unknown", position="append")
   ...
   ! write(69,*)  "A: ", A, kA, " (", curPar, ")   B: ", B, kB
    ...
    ! do m=1,2
      ! write(69,*) "m=",m, ',a=1'
      ! do x=1,7
        ! write(69,'(7f8.1)') LLA(x,:,m,1)
      ! enddo
    ! enddo
    ! write(69,*) ""
! !    write(69,'("LLtrio: ", 7f8.1)')  LLtrio
  ! close(69)
! endif
...
!if (A==189)  print*, 'maybeRev ALR: ', candP, ALR(1:2)
...
double precision :: ALR(7), LLGR(3), ALRgr(3), LLGGP(5), ALRgg(5), &
  ...
  LLZ(7), LLC(7,2), LLHH(2,2,3),  LLP(2,2), LLFA, LLPS(2,2), LLU, ALRf, ALRs, LLS
logical :: fclsib
...
!      if (focal==1)  call PairPO(A, B, k, focal, LLg(1))
      if (fclsib) then 
        ! if (Complx==0 .or. (Parent(A, 3-k)/=0 .and. Parent(A,3-k)==Parent(B,3-k))) then
          ! LLg(3) = impossible
        ! else
...
if (.not. any(Parent(A,:)==B) .and. focal/=7 .and. .not. &
  (focal==1 .and. all(Parent(A,:)==0) .and. all(Parent(B,:)==0))) then   ! if no parents, LL indistinguishable.
  do x=1,2
    do i=1,2
...
LLS = missing
ALRs = missing
if (ANY(Parent(A,:)/=0) .or. ANY(Parent(B,:)/=0)) then  ! else: could be both. 
...
if (focal==1 .and. LL(5) < LL(7)) then  ! check if FS of other parent
  ...
  if (LLFA<0D0 .and. (LL(5) - addALR(LLFA, ALRf)) < TA) then
    LLg(5) = LLFA
...
! if ((A==686 .and. B==995) .or. (A==995 .and. B==686)) then
! !if (A>643 .and. A<651 .and. B>643 .and. B<651) then
  ! open (unit=42,file="log.txt",status="unknown", position="append")
    ! write (42, *) ""
...
! if ((A==1630 .and. B==195) .or. (A==195 .and. B==1630)) then
...
!if (A==307 .and. B==309) print *, "PairFS: ", A, B, Par, ", ", AlreadyHS, LL
...
!        PrXYZ(x,y,z,3) = PrXYZ(x,y,z,3) * PrP(y,1) * PrP(z,2) ! P1+P2 GP pair
      enddo
    enddo
  enddo
  PrL(l,2) = LOG10(SUM(PrXY(:,:,2)))
  PrL(l,3) = LOG10(SUM(PrX))
!  PrL(l,4) = LOG10(SUM(PrXYZ(:,:,:,3))) 
...
! subroutine trioOff(A, kA, B, C, LL)  ! B & C both offspring of A, all >0
! use Global
! implicit none
...
! double precision, intent(OUT) :: LL
! integer :: l, x, y, k
! double precision :: PrL(nSnp), PrXY(3,3), PrA(3), PrPB(3), PrPC(3)!, LLBC
...
! ! parents are -not- ignored
! PrL = 0D0
! do l=1, nSnp
  ! PrXY = 0D0
  ! call ParProb(l, A, kA, 0, 0, PrA)
  ! call ParProb(l, Parent(B,3-k), 3-k, 0, 0, PrPB)
  ! call ParProb(l, Parent(C,3-k), 3-k, 0, 0, PrPC)
  ! do x=1,3  ! A
    ! do y=1,3  ! Parent(B,3-k)
      ! PrXY(x,y) = PrA(x) * OKA2P(Genos(l,B),x,y) * PrPB(y) * &
        ! SUM(OKA2P(Genos(l,C),x,:) * PrPC)
    ! enddo
  ! enddo
  ! PrL(l) = LOG10(SUM(PrXY))
! enddo
...
integer, intent(IN) :: A,B,k, focal
double precision, intent(OUT) :: LL
integer :: l, x, y, curGP(2,2), m, z, v, i
...
integer :: AA(maxSibSize), PA, nA, GA(2), BB(maxSibSize), nB, PB(2), BBx(maxSibSize, 2), &
  ...
  Bj, w, l, x, g, y, z, i, r,u,j,e,Ei,m
double precision :: PrL(nSnp), PrG(3,2), PrXYZ(3,3,3), PrPA(3), PrA(3), & ! PrLx(nSnp), &
...
! if ((A==-4 .and. B==-14) .and. kA==2 .and. kB==2) then
  ! print *, ""
...
double precision, intent(OUT) :: LL
integer :: PA, PB, GA(2), GB(2), GGA, l, x, y, z, v, DoQuick, m
...
!  if (Complx==0 .and. k==2) cycle
 ...
 !     if (any(SibID(:,s,k)==1465) .and. any(SibID(:,r,k)==977)) print *, "bleep! ", k, FSM, LLM(1,2), LLM(7,2)
...
double precision :: ALR, LL(7,2), dLL, LRQ
...
integer :: x, i, j, k, s, curPar(2), nCP(2), CandPar(mxCP, 2), fcl, &
   TopTmp, BYrank(nInd)
  ...
  do k=1,2
    ...
    do s=1, nC(k)
      ...
      if (Complx==0) then
        ...
        call Qadd(i, s, k, LRQ)
        if (LRQ < ns(s,k)*TF) cycle    ! TODO: check: just TF ?
      endif
      fcl = 3
      LL = missing
      LLtmp = missing
...
!        endif   ! mate may not be valid
    ...
    do m=1,2
      do r=1, nC(m) 
        ...
        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, 'bleep', m, r
        if (any(CandGP(:,m) == -r)) cycle  ! current GP
        call ChkAncest(-r,m, -s,k, AncOK)
        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, "AncOK: ", AncOK
        if (.not. AncOK)  cycle
        ...
        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, "ALR-1: ", ALRtmp(1)
        if (ALRtmp(1) == impossible .or. ALRtmp(1) < 2.0*TF) cycle
        call CalcAgeLR(-r,m, -s,k, 0,1, .TRUE., ALRtmp(2))
        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, "ALR-2: ", ALRtmp(2)
        if (ALRtmp(2)/=impossible .and. (ALRtmp(1)-ALRtmp(2)) < TF)  cycle 
        if (ALL(ABS(ALRtmp) < 0.001))  cycle  ! no age info
        if (hermaphrodites==0) then
          call QGP(-r, m, s, k,  LRG) 
!        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, "QGP: ", LRG
          if (LRG < TF*dble(MIN(nS(s,k), nS(r,m)))) cycle  ! conservative.
        endif

        LLX = missing
        call PairUA(-s, -r, k, m, LLX(1,1))
!        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, "LLX 1-1", LLX(1,1)
        if (LLX(1,1)>0) cycle
        call CalcU(-s,k, -r,m, LLX(1,2)) 
!        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, "LLX 1-2", LLX(1,2)
        if ((LLX(1,1) - LLX(1,2)) < nS(s,k)*TF) cycle
        call addFS(0, r, m, s, k, LLX(2,1), ix, dx) 
!        if (k==1 .and. s==7 .and. m==1 .and. any(sibID(:,r,m)==241))  print *, "LLX 2-1", LLX(2,1)
...
double precision :: LRHS, ALR(7), LLPH(2), ALRH(2), LLAU(2,3), ALRAU(2,3), LLUi, &
  LLC, LLz(6), ALRz(6), LLM(3), LLp(7), LLpg(7), LLFH(3), LLPX(2,2), dx(maxSibSize), &
  ALRq, LLHH(4,2), ALRtmp, LHH(3), LLy(2,2), LLpo(ns(SB,k),2), ALRpo(ns(SB,k),2), &
  LLgp(ns(SB,k),3), ALRgp(ns(SB,k),3), LLFA(ns(SB,k))!, LLgpX(3)
integer :: x, y, FSPar, i, ParTmp(2), npt, fsi, ix, m, Bi, sib1, curpar(2)                               
...
!  do x=1,3
!    call AddGPx(A,SB,k, x, LLgpX(x))  !! NEW !!
!  enddo
endif
  ...
  if (ns(SB,k)==1) then
    do Bi=1, ns(SB,k)
      ...
      if (focal==1) then
        ...
        if ((LLPH(1) > LLg(1) .and. LLPH(1)<0) .or. LLg(1)>0)   ALR(1) = ALRH(1)
        LLg(1) = MaxLL((/LLg(1), LLPH(1)/))
      endif
      if ((LLPH(2) > LLg(4) .and. LLPH(2)<0) .or. LLg(4)>0)   ALR(4) = ALRH(2)  
      LLg(4) = MaxLL((/LLg(4), LLPH(2)/))
    enddo
  endif
endif
...
if (Complx>0) then
  do x=1,2
    ...
    if (ALRAU(1,x)/=impossible .and. .not. (focal==7 .and. x==3-k .and. Parent(A,3-k)==0) .and. &
         .not. (focal==4 .and. Parent(A,x)/=0)) then  ! else conflict with CalcCandParLL
...
if (complx>0 .and. fclsib .and. hermaphrodites/=2 .and. &
 abs(MaxLL(LL(2:3)) - MaxLL(LL)) < 0.01 .and. Parent(A,3-k)==0) then 
  ...
  if (FSpar > 0) then
    ...
    ! else if (FSpar==A) then ! e.g. when BY of A unknown
      ! do i=1, nS(SB,k)
    ...
    if (ANY(Parent(SibID(1:ns(SB,k), SB, k), 3-k) < 0) .and. .not. MaybeOpp) then
      do i=1, nS(SB,k)
...
LLgp = missing
ALRgp = missing
if (focal/=7 .and. focal/=6 .and. (FSpar<0 .or. FSpar==A .or. &
...
! if ((A==42 .or. A==27 .or. A==38) .and. any(SibID(:,SB,k)==94)) then
 ! open (unit=42,file="log.txt",status="unknown", position="append")
  ! write (42, *) ""
...
! if ((any(SibID(:,SA,kA)==140) .and. ANY(SibID(:,SB,kB)==142)) .or. &
 ! (any(SibID(:,SA,kA)==142) .and. ANY(SibID(:,SB,kB)==140)))  then
  ! open (unit=42,file="log.txt",status="unknown", position="append")
    ! write (42, *) ""
    ...
    ! write(42,'("LLM ", 2i5, 5f8.1, " ; ", 2f8.1)') Par, LLM, LLM(2)-LL(7), LLM(3)-LLM(1)
...
! if (any(SibID(:,SA,k)==122) .and. ANY(SibID(:,SB,k)==102)) then
! open (unit=42,file="log.txt",status="unknown", position="append")
    ! write (42, *) ""
...
! if (A==321 .and. any(SibID(:,SB,kB)==323)) then
  ! open (unit=42,file="log.txt",status="unknown", position="append")
  ! write (42, *) ""
...
double precision :: LRQ, PrL(nSnp), PrG(3, 2), PrXY(3,3,2),  PrZ(3),  PrE(3)
...
if (Complx==0 .and. Mate(A)/=0) then  
  if (Mate(A) /= curGP(3-m) .and. curGP(3-m)/=0) then
    LL = impossible
    return
...
! if (A==95 .and. any(SibID(:,SB,k)==166))  then
  ! print *, "AddGP: ", A, Parent(A,:), " + ", SB, k, "; cats:", cat, catG, DoQuick
! endif
...
! subroutine AddGPx(A, SB, k, hf, LL)   ! GPs of SB are A + HS/FS of A 
! use Global
! implicit none

! integer, intent(IN) :: A, SB, k, hf
! double precision, intent(OUT) :: LL
! integer :: l, x, y, z, v, w, n, DoQuick
! double precision :: PrL(nSnp), PrXY(3,3,3,3,3), PrPA(3,2), PrB(3)
...
! call ChkDoQuick(SB,k,DoQuick) 
! if (DoQuick==-1 .or. DoQuick==-3) then
  ! LL = NotImplemented
  ! return
! endif
...
! do l=1,nSnp
  ! do n=1,2
    ! call ParProb(l, Parent(A,n), n, A, 0, PrPA(:,n))
  ! enddo
  ! PrB = XPr(1,:,l,SB,k)
  ...
  ! do x=1,3  ! SB
    ! do y=1,3  ! A
      ! do z=1,3  ! FS of A
        ! do v=1,3
          ! do w=1,3
            ! PrXY(x,y,z,v,w) = PrB(x) * AKA2P(x,y,z) * OcA(Genos(l,A),y) * &
              ! AKA2P(y,v,w) * PrPA(v,1) * PrPA(w,2)
            ! if (hf==1) then
              ! PrXY(x,y,z,v,w) = PrXY(x,y,z,v,w) * AKAP(z,v,l)
            ! else if (hf==2) then
              ! PrXY(x,y,z,v,w) = PrXY(x,y,z,v,w) * AKAP(z,w,l)
            ! else if (hf==3) then
              ! PrXY(x,y,z,v,w) = PrXY(x,y,z,v,w) * AKA2P(z,v,w)
            ! endif  
          ! enddo
        ! enddo
      ! enddo
...
! ! if (A==42 .and. any(SibID(:,SB,k)==94))  then
  ! ! print *, "AddGPX: ", A, Parent(A,:), " + ", SB, k, hf, " Quick:", DoQuick, LL
! ! endif

! end subroutine AddGPx
...
!  LL = SUM(PrL) + Lind(A) - Lind(GG)   ! No idea why, but do not subtract Lind(GG)
else
  LL = SUM(PrL) + Lind(A)
endif

! if (A==63 .and. any(SibID(:,SB,k)==99) )  then
...
double precision, intent(OUT) :: LL
integer :: PA, nA, AncA(2,mxA), AA(maxSibSize), G(2), AncB(2,mxA), GA, GB, &                                                          
...
! if (SA==11 .and. SB==9 .and. kB==1) then
  ! print *, ""
...
double precision :: LLGX(2,2), LLtmp(maxSibSize, 2), PrL(nSnp), PrZ(3), &
  PrG(3), PrPG(3), PrXYZ(3,3,3,3,2), PrW(3), dx(maxSibSize), PrH(3)
...
! if (SA==2 .and. kA==2 .and. SB==5 .and. kB==1) then
...
! if (ALL(LPBY(:,3:5) < -HUGE(0D0))) then
  ! open (unit=42,file="log.txt",status="unknown", position="append")
  ! write (42, *) ""
  ! if (A>0) then
    ! write (42, *) "LPBY ", A, Parent(A,:)  
  ! else
    ! write (42, *) "LPBY ", A, GpID(:,-A,k) 
    ! write(42,*)  "Off: ", SibID(1:ns(-A,k),-A,k)
  ! endif
 ! ! write(42,'(5x, 50i8)')  (/ (w, w=BYzero+36, BYzero+nYears) /)
  ! write (42, '(i5, 50f8.2)') 5, 10**LPBY(:,5)
  ! write (42, *) ""
    ! write (42, '("self ", 50f8.2)') 10**LPBY(:,1)
    ! write (42, '("up   ", 50f8.2)') 10**BYup(:,2)
    ! write (42, '("down ", 50f8.2)') 10**BYdown(:,2)
    ! write (42, '("sibs ", 50f8.2)') 10**BYsibs(:)
    ! close(42)
    ! stop
! endif
...
do m=1,2
  ...
  do g=1,2
    if (GP(m,g) > 0)  Ylast = MIN(Ylast, YearLast( GP(m,g) ) +MaxAgePO )  
  enddo
enddo
...
do y=2,nYears  
  do m=1,2
    ...
    if (y > Ylast) then
      ...
      tmpX = 0D0
      do x=1, y-1 
        tmpX(x) = BYP(x,m) * 10**getAP(y-x, 1, m, 0)  ! parent born in year x
      enddo
    ...
    do g=1,2
      ...
      if (ANY(BYG(:,m,g)>=1D0)) then  
        ...
        BYA(y,:) = BYA(y,:) + getAP(y - tmpBY, 4, m, g)
      else
        tmpX = 0D0
        do x=1, y-2 
          tmpX(x) = BYG(x,m,g) * 10**getAP(y-x, 4, m, g)  ! grandparent born in year x
        enddo 
        BYA(y,2) = BYA(y,2) + LOG10(SUM(tmpX))
      endif
    enddo
...
! if (A==118 .and. BYA(44,2) < -3.0) then
  ...
  ! write (42, '("down1", 50f8.2)') 10**BYA(36:nYears,1)
  ! write (42, '("down2", 50f8.2)') 10**BYA(36:nYears,2)
  ! close(42)
! !  stop
! endif
...
do m=1,2
  do i=1, nSibs(m)
    if (Sibs(i,m) == A) then
      Sibs(i,m) = 0
    else if (Parent(Sibs(i,m),1) == Par(1) .and. Parent(Sibs(i,m),2) == Par(2)) then
      if (m==1) then
        nSibs(3) = nSibs(3) +1
    ...
    do i=1, nSibs(m)
      ...
      if (BY(Sibs(i,m)) > 0) then  ! sibling i has known BY
        ...
        if (m < 3) then  ! half sibs
          BYA(y) = BYA(y) + getAP(AgeD, 3, 0, m)
        else  ! full sibs
          BYA(y) = BYA(y) + getAP(AgeD, 2, 0, 0)
        endif
      endif
    enddo
  enddo
...
! if ((A==-7 .and. kA==1 .and.  B==-26 .and. kB==1 .and. focal==1) .or. &
 ! (A==-26 .and. B==-7 .and. kA==1 .and. kB==1 .and. focal==1)) then
  ! open (unit=42,file="log.txt",status="unknown", position="append")
  ! write (42, *) ""
  ! write(42,'("ALR CAU ", 2i5," ,",2i5,": ", f9.3, i3)') kA, A, kB, B, ALR, m
   ! do i=1,2
     ! write (42, '("BYLR ", i5, 50f10.2)') AB(i), BYLR(:,i)
  ! enddo
  ! write (42, *) ""   
  ! do y=1,nYears !-1
    ! if (ALL(ALRtmp(:,y) == LOG10(zero)))  cycle
    ! write(42, '("ALRtmp ", i6, 50f10.3)') BYzero + y, ALRtmp(:, y) !, COUNT(ALRtmp(:,y) >0)
  ! enddo
  ! write (42, *) ""   
  ! close(42)
! endif
...
integer :: nFSi, FSi(MaxSibSize), Inbr(2), Anc(2,mxA), PIK, l, x, y, k, z, j
   !, nOff, Off(MaxSibSize), sxOff(MaxSibSize), r
double precision :: PrL(nSnp), Px(3,2), PrG(3), PrXYZ(3,3,3), PrX(3) !, PrE(3)
logical :: Selfed
...
do l=1,nSnp
  ...
  ! if (all(LindX(:,l,i) == 0D0) .or. any(LindX(:,l,i) /= LindX(:,l,i))) then 
    ! print *,""
    ! print *, i, ", ", Parent(i,:), "Geno: ", Genos(l,i)
    ! print *, "Inbr: ", any(Inbr==1), ", Selfed: ", SelfedIndiv(i), ", doFS: ", doFS
    ! print *, ""
    ! write(*, '(i3, " PrX: ", 3f5.2)') l, PrX
    ! print *, ""
    ! do k=1,2
      ! write(*, '(i3, " Px: ", 3f5.2)') k, Px(:,k)
    ! enddo
    ! call ErStop("LL-ind error", .TRUE.)
  ! endif
...
do l=1,nSnp
  ...
  ! if (all(XPr(3,:,l,s,k) == 0D0)) then 
    ! print *,""
    ! print *, l, "-- ", k, s, " ; ", SibID(1:4, s, k), "; ", GpID(:,s,k)
    ! print *, ""
    ! do r=1, ns(s,k)
      ! write(*, '(i4, 2i7, " PrYp ", 3f5.2, 2i4, i7)') r, Sibs(r), Parent(Sibs(r),3-k), PrYp(:,r), &
        ! Isinbr(r), UseEE(r), MatePar(r)
    ! enddo
    ! ! print *, ""
    ! ! print *, l, " LPrX-1: ", LPrX(:,1)
    ! ! print *, l, " LPrX-2: ", LPrX(:,2)
    ! print *, ""
    ! do v=1,2
      ! do x=1,3
        ! print *, x, v, " PrXZ: ", PrXZ(x,:,v)
      ! enddo
       ! print *, ""
    ! enddo
    ! call ErStop("meh")
...
integer :: k,i,m, numcolLH, nDupLhID, j, IOerr, dumI(5)
...
do k=1, nIndLH
  if (numcolLH==3) then
    ...
    read(103,*,IOSTAT=IOerr) dumC, dumI(1:4)
